<template>
  <div id="user-list">
    ...
    <manage-user-edit-user
      ...
      :propAbilityList="customer.ability"
...
import ManageUserEditUser from './ManageUserEditUser.vue'
  ...
  methods: {
    async getListSelect(){
      try {
        ...
        let resC = await this.$http.get(`custumer/custumer/${this.userData.custumerID}`)
        console.log(resC)
        this.customer = resC.data.data
      } catch (error) {
        console.error(error)
      }
    },
